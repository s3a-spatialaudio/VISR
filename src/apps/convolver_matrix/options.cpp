 ...
 : apputilities::Options()
{
  ...
  registerOption<std::size_t>( "period,p", "Period (blocklength): The number of samples per audio block, also the block size of the partitioned convolution." );
  ...
  registerOption<std::string>( "filters,f", "Initial impulse responses, specified as comma-separated list of one or multiple WAV files." );

  registerOption<std::string>( "filter-file-index-offsets", "Index offsets to address the impulses in the provideed multichannel filter files."
    " If specified, the number of values must match the number of filter files." );

  registerOption<std::string>( "routings,r", "Initial routing entries, provided" );
