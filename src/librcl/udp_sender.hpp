 ...
 * The message can operate either synchronously (messages are sent when the
 * process() method is called) or asynchronously (the messages queued for
 * sending and then send non-blocking in an extra thread.
 */
...
public:
  enum class Mode
  {
    Synchronous,  /**< The messages are sent within the process() call, blocking
                     the call*/
    Asynchronous, /**< The messages are stored in a queue, and send
                     non-blocking, asynchronously when the network stack is
                     ready. */
  };
