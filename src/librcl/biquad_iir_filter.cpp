...
void BiquadIirFilter::process()
{
  ...
  for( std::size_t sampleIdx( 0 ); sampleIdx < blockSamples; ++sampleIdx )
  {
         ...
         ++biquadIdx )
    {
      ...
      // Note: this expects that the denominator coefficients a1 and a2 are
      // already negated. v0 = b0 * x[n] + v1 y[n] = v0 v1 = a1*y[n] + b1 * x[n]
      // + v2 v2 = a2*y[n] + b2 * x[n] Coefficient order in mCoefficients rows:
      // [b0 b1 b2 a1 a2]
      efl::ErrorCode res;
      // y[n] = b0 * x[n] + v1
      if( ( res = efl::vectorMultiplyAdd(
                mCoefficients.row( cNumBiquadCoeffs * biquadIdx ),
                mCurrentInput.data(), mState.row( 2 * biquadIdx ),
                mCurrentOutput.data(), mNumberOfChannels,
