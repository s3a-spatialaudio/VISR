 ...
 * @param modulePath Optional search path for the location of the module. If left empty, the default Python system path (including the value of <tt>$PYTHONPATH</tt>) is searched. The paths are also appended to the system path, for example to locate dependencies of the module \p moduleName. 
 ...
 * sys.path is searched, which includes the value of <tt>$PYTHONPATH</tt>. otherwise the value of \p modulePath is appended to <tt>sys.path</tt>, for example to locate dependencies of \p modulePath.
 * @param globals Any variables or definitions to be passed to the Python interpreter.
 * @return A Python <b>module</b> object.
