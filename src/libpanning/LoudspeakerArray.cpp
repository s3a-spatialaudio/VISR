	...
	namespace panning
	{
		...
		void LoudspeakerArray::loadXmlStream(std::istream & fileStream)
		{
			...
			for (ptree::const_assoc_iterator treeIt(speakerNodes.first); treeIt != speakerNodes.second; ++treeIt, i++)
			{
				...
				m_id[id] = i;
				//std::cout << "KEY: " << id<< " VAL: "<<m_id[id] << std::endl;
				m_channel[i] = chIdx - 1;
			...
			for (ptree::const_assoc_iterator tripletIt(tripletNodes.first); tripletIt != tripletNodes.second; ++tripletIt, j++)
			{
				...
				//std::cout << "TID1 " << tid1 <<" VAL "<< m_id[tid1]<< " TID2: " << tid2 << " VAL " << m_id[tid2]  << " TID3: " << tid3  << " VAL " << m_id[tid3]<<std::endl;
				...
				/*if (m_id.find(tid1) == m_id.end()) std::cout << "WRONG1 " << std::endl;
				if (m_id.find(tid2) == m_id.end())std::cout << "WRONG2 " << m_id[tid2]<< std::endl;
				if (tid1.empty() or tid2.empty())std::cout << "WRONG3 " << std::endl;
				*/
				triplet[0] = m_id[tid1];
				triplet[1] = m_id[tid2];
				if (m_is2D)
			...
			for (ptree::const_assoc_iterator subIt(subwooferNodes.first); subIt != subwooferNodes.second; ++subIt, ++subIdx)
			{
				...
				//boost::trim_if(speakerIndicesStr, boost::is_any_of("\t "));
				...
				{
					throw std::invalid_argument("At least one of the specified loudspeaker index does not exist.");
				}
