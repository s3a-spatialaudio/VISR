...
namespace visr
{
  ...
  void VBAP::setListenerPosition( SampleType x, SampleType y, SampleType z )
  {
    ...
    calcInvMatrices();
		...
		void VBAP::calcGain(XYZ pos) {
			std::fill(mGain.begin(), mGain.end(), 0);
			LoudspeakerArray::TripletType tri;
			calcPlainVBAP(pos);
			applyRerouting();
			normalise(mGain);
		}

		void VBAP::calcPlainVBAP(XYZ pos) {
