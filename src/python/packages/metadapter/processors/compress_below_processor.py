# Compress any negative height (below) component of all objects
...
class CompressBelowProcessor(SequenceProcessorInterface):
	...
	def processObjectVector( self, objectVector):
		...
		for obj in objectVector:
			...
			if self.active:
				if obj_type == 'plane':
					if float(obj['direction']['el']) < 0.0:
						obj['direction']['el'] = 0.0
				elif obj_type == 'point':
					if 'x' in obj['position'].keys():
						if float(obj['position']['z']) < 0.0:
	...
	def setParameter( self, key, valueList ):
		...
		if (key == "active"):
			if len( valueList ) != 1:
				raise KeyError( "CompressBelowProcessor command \"active\" must contain 1 value." )
