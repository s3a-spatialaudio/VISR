...
# <af> Removed ForkingMixIn because it is not supported by recent versions of socketserver.
# <af> In turn that means that the 
from socketserver import UDPServer, DatagramRequestHandler, ThreadingMixIn # , ForkingMixIn
...
# Replacement for Python 3 types.StringTypes, including plain and Unicode strings
global StringTypes
...
class OSCMessage(object):
	...
	def clearData(self):
		...
		"""
         # Explicitly create 'bytes' data instead of Unicode str
		self.typetags = b','
		self.message  = b''
	...
	def append(self, argument, typehint=None):
		...
		if type(argument) == dict:
         ...
         # Seems to be a difference between Python 2 and 3: Literal strings are iterable in Python3
		if hasattr(argument, '__iter__') and not type(argument) in StringTypes:
			for arg in argument:
	...
	def tags(self):
		...
		"""
		return list(self.typetags.lstrip(b','))
...
class OSCBundle(OSCMessage):
	...
	def _reencapsulate(self, decoded):
		if decoded[0] == b"#bundle":
			...
			tags = decoded[1].lstrip(b',')
			for i in range(len(tags)):
				msg.append(decoded[2+i], bytes([tags[i]]) )
...
def _readString(data):
	...
	# TODO: Do we need to care about exceptions thrown if \0 is not found?
...
# TODO: Remove this private function, which is not called from within this file.
#def _readLong(data):
...
def decodeOSC(data):
	...
	"""
 	# TODO: Consider replacing the keys by their int representation ord(b'x') and change the
	# table lookup accordingly
	table = {b'i':_readInt, b'f':_readFloat, b's':_readString, b'b':_readBlob}
	decoded = []
	...
	if address == b"#bundle":
		...
		if typetags.startswith(b','):
			for tag in typetags[1:]:
				value, rest = table[bytes([tag])](rest)
...
def hexDump(bytes):
	...
	# Contrary to the parameter name, this expects a str object ()
    # TODO: Consider changing it to work on bytes data internally, and provide
    # an conversion from str where needed.
	bytes = str(bytes)
...
# <af> Disabled ForkingOSCServer because the required functionality (ForkingMixIn) has been removed from socketserver. 
...
if __name__ == "__main__":
	...
	op.set_defaults(listen="localhost:%d" % default_port)
