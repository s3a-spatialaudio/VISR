...
def test_compositeFeedthrough(plot=False):
    ...
    inSig = np.asarray(np.cos(fSig[:, np.newaxis] * 2 * np.pi * t[np.newaxis, :]), dtype=np.float32)
    ...
    assert np.all(np.abs(outSig[:, period:] - inSig[:, :-period]) <= 5 * np.finfo(np.float32).eps)
